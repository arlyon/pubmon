...
export const pubmonMachine = setup({
	...
	actors: {
		...
		pickNameActor: fromPromise<
			any,
			{ socket: PartySocket; sessionId: string; name: string }
		>(async ({ input }) => {
		...
		createPlayerActor: fromPromise<
			any,
			{ socket: PartySocket; sessionId: string; playerInfo: PlayerInfo }
		>(async ({ input }) => {
		...
		pickPokemonActor: fromPromise<
			any,
			{ socket: PartySocket; sessionId: string; pubmonId: number }
		>(async ({ input }) => {
		...
		encounterActor: fromPromise<
			any,
			{ socket: PartySocket; sessionId: string; drinkType: PubType }
		>(async ({ input }) => {
		...
		battleResolutionActor: fromPromise<
			any,
			{
				socket: PartySocket;
				sessionId: string;
	...
	actions: {
		...
		updateTournamentBracket: assign({
			tournamentState: ({ context, event }) => {
				if (
					event.type === "TOURNAMENT_STARTED" ||
					event.type === "BRACKET_UPDATE"
				) {
					return {
			...
			playerInfo: ({ event }) => {
				if (event.type === "PLAYER_CREATED" && event.existingState) {
					return {
						...
						gender: (event.existingState.info.sprite === "boy"
							? "boy"
							: "girl") as "boy" | "girl",
					};
				}
	...
	context: ({ input }) => ({
		...
		playerInfo: input.initialPlayerState
			? {
					...
					gender:
						input.initialPlayerState.info.sprite === "boy" ? "boy" : "girl",
				}
			: null,
		party: input.initialPlayerState?.party || [],
		activeIndex: input.initialPlayerState?.activeIndex || 0,
		badges: input.initialPlayerState
			? new Set(input.initialPlayerState.badges)
			: new Set(),
		currentGymId: input.initialGymId || 1,
		gamePhase: input.initialGamePhase || "collection",
		leaderboard: [],
		activeEncounter: {
			wildPubmon: null,
			battleStartTime: null,
						...
						selectingStarter: {
							invoke: {
								...
								input: ({ context, event }) => ({
									...
									sessionId: context.sessionId,
									pubmonId:
										event.type === "SELECT_STARTER" ? event.pokemon.id : 0,
								}),
								onDone: {
									target: "#pubmon.view.mainLoop.crawl",
									actions: assign({
						...
						settingActiveMon: {
							invoke: {
								...
								input: ({ context, event }) => ({
									...
									sessionId: context.sessionId,
									activeIndex:
										event.type === "SET_ACTIVE_MON" ? event.index : 0,
								}),
								onDone: {
									target: "team",
									actions: assign({
										activeIndex: ({ event }) =>
											event.output.playerState?.activeIndex || 0,
									}),
								},
								onError: "team",
							},
										...
										FAINT_DETECTED: {
											...
											actions: assign({
												tournamentState: ({ context }) => ({
													...context.tournamentState,
													activeBattle: null, // Clear battle when done
												}),
											}),
										},
									},
						...
						resolvingEncounter: {
							invoke: {
								...
								input: ({ context, event }) => ({
									...
									sessionId: context.sessionId,
									drinkType:
										event.type === "ORDER_DRINK" ? event.drinkType : "beer",
								}),
								onDone: {
								...
								onDone: [
									...
									},
									{
										...
										actions: assign({
											...
											awardedBadgeId: ({ event }) =>
												event.output.awardedBadgeId,
							...
							states: {
								...
								xpGain: {
									on: {
										CONTINUE: {
											target: "#pubmon.view.mainLoop.crawl",
											actions: ["clearEncounter", assign({ xpGained: 0 })],
										},
									},
								},
