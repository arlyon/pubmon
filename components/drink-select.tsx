...
const DRINK_TYPES: { type: PubType; label: string; element: string; icon: string[]; color: string }[] = [
  {
    type: "beer",
    label: "BEER",
    element: "Earth Type",
...
function DrinkIcon({ icon, color, size = 4 }: { icon: string[]; color: string; size?: number }) {
  return (
    <svg viewBox="0 0 8 8" width={size * 8} height={size * 8} style={{ imageRendering: "pixelated" }}>
      {icon.map((row, y) =>
        [...row].map((cell, x) => {
          ...
          return <rect key={`${x}-${y}`} x={x} y={y} width={1} height={1} fill={fillColor} />
        })
      )}
    </svg>
  )
...
export function DrinkSelect({ onSelect, drinksCollected, currentGymId, badges, onSelectGym }: DrinkSelectProps) {
  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-md mx-auto">
      ...
      {/* Drink type grid */}
      <PixelBox>
        <p className="text-[8px] text-muted-foreground mb-3">SELECT DRINK TYPE</p>
          ...
          {DRINK_TYPES.map((drink, idx) => (
            ...
            >
              <div
                ...
                style={{ borderColor: drink.color, background: `${drink.color}22` }}
              >
                <DrinkIcon icon={drink.icon} color={drink.color} size={4} />
              </div>
              <div className="flex-1">
                <p className="text-[10px] text-foreground">{drink.label}</p>
                <p className="text-[8px] text-muted-foreground">{drink.element}</p>
              </div>
              {idx === selectedIdx && (
                <div className="text-primary text-[10px] animate-pulse">{">"}</div>
              )}
            </button>
          ))}
        </div>
      ...
      <PixelBox variant="info">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ borderColor: selected.color, background: `${selected.color}33` }}
          >
            <DrinkIcon icon={selected.icon} color={selected.color} size={5} />
          </div>
          <div className="flex-1">
            <p className="text-[10px] text-foreground">{selected.label}</p>
            <p className="text-[8px] text-muted-foreground mt-1">
              {selected.type === "beer" && "You might encounter Hoppsin or Lagerite!"}
              {selected.type === "shot" && "You might encounter Absintile, Tequilar, or Cazcabella!"}
              {selected.type === "wine" && "You might encounter Charderan or Merlot!"}
              {selected.type === "water" && "You might encounter Springer, Stillbar, Sparklid, or Tonica!"}
              {selected.type === "cocktail" && "You might encounter Martini, Aperol, or Caiprinha!"}
            </p>
          </div>
        </div>
      </PixelBox>
      ...
      {/* Bottom text */}
      <PixelBox variant="menu">
        <p className="text-[8px] text-muted-foreground text-center leading-relaxed">
          {"A wild PubMon appears when you"}
          <br />
          {"order a drink! Choose wisely..."}
        </p>
      </PixelBox>
    </div>
  )
